...
if [ ${#sampbams[@]} -eq 0 ]; then
	echo "WARNING: No bams detected for ${sampleid}" >> ${logfile} 2>&1
elif [ ${#sampbams[@]} -eq 1 ];	then
	echo "${sampleid} was not multiplexed. Renaming ${sampbams[0]} to ${final}" >> ${logfile} 2>&1
	cp ${sampbams[0]} ${final}
else
	echo "${sampleid} was multiplexed. Merging ${sampbams[@]} bams into ${final}" >> ${logfile} 2>&1
